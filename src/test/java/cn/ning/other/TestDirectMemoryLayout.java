 ...
 * run with -Xms4g -Xmx4g
 *
 * 对比堆内内存：
 * 1.性能更好，快了至少一个数量级，堆内内存方式随着GC进行堆内内存会变得碎片化，访问变得随机，性能会下降
 * 2.更紧凑的数据结构，
 *   堆外内存数据结构占用：8+8+4+4+8+8+2=42 bytes
 *   JVM数据结构占用：56 bytes（实际上会更多，使用了数组结构）
 * 3.能够处理巨大的数据集，同时避免GC停顿
 *
 * 在大数据集、吞吐量、延迟方面有极高性能要求时，堆外内存方式优势会很明显
 *
 */
public class TestDirectMemoryLayout {
    ...
    //Memory 4,116,185,088 total, 4,019,161,128 free
    //0 - duration 1373ms
    //buyCost = 6958024115266225536 sellCost = 6959274115241225536
    //Memory 4,116,185,088 total, 4,072,335,944 free
    //1 - duration 1178ms
    //buyCost = 6958024115266225536 sellCost = 6959274115241225536
    //Memory 4,116,185,088 total, 4,093,830,296 free
    //2 - duration 1171ms
    //buyCost = 6958024115266225536 sellCost = 6959274115241225536
    //Memory 4,116,185,088 total, 4,093,830,296 free
    //3 - duration 1152ms
